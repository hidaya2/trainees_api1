...
const TraineesContextProvider = ({children}) => {
    ...
    async function getTrainees() {
        const res= await axios.get("https://traineesapi-hidaya.herokuapp.com/api/v1/trainees")
        try{
            dispatch({
    ...
    return (
        <TraineesContext.Provider value={{getTrainees,trainees:state.trainees,loading:state.loading}}>
            {children}
